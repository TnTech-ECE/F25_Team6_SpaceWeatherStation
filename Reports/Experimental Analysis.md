...
| TEC Measurement Accuracy | Comparison of calculated TEC to reference data | Mean percent error ≤ 15% | Fail |
| Signal Quality (Carrier-to-Noise Density Ratio) | Quality of GNSS signals received | ≥ 70% of carrier-to-noise density values ≥ 30 dB-Hz | Pass |
...
- __TEC Measurement Accuracy__: The TEC Measurement Accuracy criterion was classified as a fail, as shown in Table [1]. The system was required to achieve a mean percent error less than or equal to 15% when compared to reference data. Comparison of the system-generated TEC values with reference data shows that, while the overall trend is loosely followed, there are significant deviations in magnitude throughout the test period. The calculated mean percent error was approximately 70%, indicating that the system did not meet the required accuracy threshold. Although the system captures general variations in TEC, the large discrepancies suggest that the measurements lack sufficient precision for accurate measurement of ionospheric conditions. A primary contributing factor to this performance is the test environment. Data for this trial was collected indoors with the system positioned near a window, allowing partial satellite visibility. However, this setup likely introduced signal attenuation and multipath interference, degrading pseudorange measurements and reducing TEC accuracy. To address this limitation, a follow-up test is being conducted with the system deployed in an improved outdoor environment over a 24-hour period. This test is expected to provide higher-quality signal reception and a more accurate assessment of system performance under proper operating conditions.
